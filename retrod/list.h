#pragma once
#include <stdlib.h>
#define LIST(type) struct type##List                                    // TODO: Add pointer inside
#define DECLARE_LL(type) LIST(type) { type data; LIST(type)* next }
#define _LL_ELEMENT(type, data, next) ({ LIST(type)* _elem_ = malloc(sizeof(LIST(type))); *_elem_ = { data, NULL }; _elem_ })       // Fix maybe
